  ...
  const columns = [
    {
      ...
      render: (loc) => {
        ...
        return src ? (
          ...
          <div className="flex h-12 w-16 items-center justify-center rounded-lg bg-gray-100 text-[10px] text-gray-400">—</div>
        )
      },
    },
    {
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {isAdmin ? (
            <Button
              ...
              onClick={() => {
                if (window.confirm('Supprimer ce lieu ? Impossible s’il est lié à des réservations.')) {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        <div className="min-w-0">
          <h1 className="text-2xl font-bold text-secondary sm:text-3xl">Points de collecte</h1>
          <p className="text-gray-500">Lieux affichés sur le site et proposés aux réservations (adresses FR / AR, photos).</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <StatsCard title="Lieux" value={String(rows.length)} icon={MapPin} badgeText="Site public" badgeVariant="neutral" />
      </div>

      <DataTable columns={columns} data={rows} isLoading={isLoading} emptyMessage="Aucun lieu." />
    </motion.div>
  )
}
