...
export default function BlogListPage() {
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(t('page.blog.toastPublish'))
    },
    onError: (err) => toast.error(err.response?.data?.error || t('page.blog.toastErr')),
  })
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['blog'] })
      toast.success(t('page.blog.toastDeleted'))
    },
    onError: (err) => toast.error(err.response?.data?.error || t('page.blog.toastDeleteErr')),
  })
  ...
  const columns = useMemo(
    () => [
      ...
      {
        key: 'category',
        label: t('page.blog.colCategory'),
        render: (p) => p.category || '—',
      },
      {
        key: 'status',
        label: t('page.blog.colStatus'),
        render: (p) => (
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {p.is_published ? t('page.blog.online') : t('page.blog.draft')}
            </span>
            {p.is_featured && (
              <span className="inline-flex items-center gap-0.5 text-amber-600" title={t('page.blog.featuredTitle')}>
        ...
        render: (p) => (
          <div className="flex flex-wrap gap-2">
            <Button size="sm" variant="ghost" type="button" onClick={() => toggleMutation.mutate(p.id)}>
              {p.is_published ? t('page.blog.unpublish') : t('page.blog.publish')}
            </Button>
            ...
            {canDelete && (
              <Button
                ...
                onClick={() => {
                  if (window.confirm(t('page.blog.deleteConfirm'))) deleteMutation.mutate(p.id)
                }}
              >
                {t('common.delete')}
              </Button>
    ...
    [t, navigate, canDelete, toggleMutation, deleteMutation]
  )
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        <div className="min-w-0">
          <h1 className="text-2xl font-bold text-secondary sm:text-3xl">{t('page.blog.title')}</h1>
          <p className="text-gray-500">{t('page.blog.subtitle')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <StatsCard
          title={t('page.blog.statsTitle')}
          value={String(rows.length)}
          icon={FileText}
          badgeText={t('page.blog.statsBadgePublished', { count: publishedCount })}
          badgeVariant="neutral"
        />
        <Card className="flex items-center p-4 text-sm text-gray-600">{t('page.blog.slugHint')}</Card>
      </div>
      ...
      <DataTable
        ...
        emptyMessage={t('page.blog.empty')}
      />
    </motion.div>
  )
}
